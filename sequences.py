...
def generate_return_sequences(retname):
	...
	codestr += "ld x1, (x2) #return pc from the stack\n"  #return pc from the stack
...
def gen_cbeqz_always_taken_forward():
	...
	codestr = ""
	loop_length = random.randint(2,4) #how many instructions does the loop have?
...
def gen_cbnez_always_taken_backward():
	...
	codestr = ""
	loop_length = random.randint(2,10) #how many instructions does the loop have?
...
def gen_loads_and_stores():
	...
	for i in range(num):
		...
		ld_flavor = ["lb","lh","lw","ld"]
		st_flavor = ["sb","sh","sw","sd"]
...
def gen_unconditional_jmp():
	...
	codestr = ""
	jump_with_link = 0
	loop_length = random.randint(2,10) #how many instructions to skip
...
def gen_call():
	...
	if ret_info and ret_info!=ret_info_exclude:
		...
		codestr += f"sd {src}, (x2)   #update stack pointer with return address\n" #update stack pointer with return address
		codestr += f"li {src}, {hex(address)}\n"
